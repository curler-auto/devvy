...
/**
 * JSON to TOML Converter Tool
 * Convert JSON to TOML format
 * Client-side implementation
 */
function JSONToTOML({ tab, tabs, setTabs, editorTheme = 'vs-dark' }) {
  ...
  return (
    <div className="json-to-toml-tool h-full flex flex-col" data-testid="json-to-toml">
      <div className="flex items-center justify-between p-4 border-b border-[var(--border-primary)] bg-[var(--bg-secondary)]">
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">JSON to TOML Converter</h2>
          ...
          <div className={`flex flex-col border-r border-[var(--border-primary)] ${isInputFullscreen ? 'w-full' : 'w-1/2'}`}>
            <div className="flex items-center justify-between p-3 border-b border-[var(--border-primary)] bg-[var(--bg-secondary)]">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={formatJSON} size="sm" variant="ghost" className="h-8"><Code className="w-4 h-4 mr-2" />Format</Button>
