...
const PDFScanTool = () => {
    // Scan PDF tool does not need a file upload initially, so we don't use the wrapper in the standard way
    // or we can make the wrapper support a "no file" mode.
    // However, toolconfig usually implies clicking tool -> opening component.
    // We will return ScanPDF directly but wrap it in a div to match layout.
    // We mock onClose to just reload or something, or better yet, inject onClose from parent if possible.
    // For now, we'll just implement basic structure.
    return (
        <div className="h-full w-full">
            <ScanPDF onClose={() => window.location.reload()} />
        </div>
    );
};

PDFScanTool.metadata = {
  id: 'pdf-scan',
  name: 'Scan to PDF',
  description: 'Capture documents from camera',
  category: 'image',
  requiresBackend: false,
};

export default PDFScanTool;
